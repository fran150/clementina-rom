...
func TestBasicFSExtensionsUpdateAndQueries(t *testing.T) {
	...
	lines := []string{
		...
		`60 FLUSH #1`,
		`70 POKE 24576,FPOS(1)`,
		`80 POKE 24577,FSIZE(1)`,
		`90 POKE 24578,FPOS(2)`,
		`100 POKE 24579,FSIZE(2)`,
		`110 BGET#1,V:POKE 24580,V`,
		`120 CLOSE #1:CLOSE #2`,
		`130 OPEN "C" FOR UPDATE AS #3`,
		`140 POKE 24581,FSIZE(3)`,
	...
	for i, v := range []int{2, 5, 1, 3, 67, 0, 42} {
		if int(peek(c, uint32(24576+i))) != v {
			dumpScreen(t, c.chips.mia.(videoReader))
		}
		require_eq(t, v, peek(c, uint32(24576+i)), fmt.Sprint("query ", i))
	}
...
func TestBasicFSExtensionsMetadataAndFree(t *testing.T) {
	...
	for _, l := range []string{
		`10 DIM Z(3)`,
		`20 FSTAT "A",S,A,D,T`,
		`30 IF S=257 THEN POKE 24576,1`,
		`40 IF D>0 THEN POKE 24577,1`,
		`50 FSTAT "D",Z(0),Z(1),Z(2),Z(3)`,
		`60 POKE 24578,Z(1) AND 16`,
		`70 F=DISKFREE(0)`,
		`80 IF F=1073709056 THEN POKE 24579,1`,
		`90 POKE 24580,42`,
		`100 GOTO 100`,
	} {
		typeLine(c, step, l)
	...
	for i, v := range []int{1, 1, 16, 1, 42} {
		...
		require_eq(t, v, peek(c, uint32(24576+i)), fmt.Sprint("metadata ", i))
	}
}
...
func TestBasicFSExtensionsSeek32(t *testing.T) {
	...
	for _, l := range []string{
		...
		`20 N=4294967295`,
		`30 IF FSIZE(1)=N THEN POKE 24576,1`,
		`40 SEEK#1,2147483648`,
		`50 P=FPOS(1)`,
		`60 IF P=2147483648 THEN POKE 24577,1`,
		`70 SEEK#1,N`,
		`80 IF FPOS(1)=N THEN POKE 24578,1`,
		`90 SEEK#1,16777217.9`,
		`100 P=FPOS(1)`,
		`110 IF P=16777217 THEN POKE 24579,1`,
		`120 CLOSE #1`,
		`130 POKE 24580,42`,
		`140 GOTO 140`,
	} {
	...
	for i, v := range []int{1, 1, 1, 1, 42} {
		...
		require_eq(t, v, peek(c, uint32(24576+i)), fmt.Sprint("32-bit seek ", i))
	}
}
