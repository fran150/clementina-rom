...
func TestBasicTiming(t *testing.T) {
	...
	for _, s := range []string{
		...
		"50 IF B<80 THEN POKE 512,1", "60 IF TI<4 THEN POKE 512,2",
		"70 TI=3600", "80 IF TI<3600 THEN POKE 512,3",
		"90 IF TI>3610 THEN POKE 512,4", "100 DELAY 0",
		"110 POKE 513,123", "120 GOTO 120",
	} {
		typeLine(c, step, s)
	}
	typeLine(c, step, "RUN")
	editorTickN(c, step, 3_000_000)
	require_eq(t, 0, peek(c, 512), "timing bounds")
	require_eq(t, 123, peek(c, 513), "completed delay and TI assignment")
}
func TestBasicTimingBreak(t *testing.T) {
	c, step := bootClementinaToPrompt(t)
	typeLine(c, step, "10 DELAY 60000")
	typeLine(c, step, "20 POKE 512,123")
	typeLine(c, step, "RUN")
	injectKeys(c, 3)
	...
	require_eq(t, 0, peek(c, 512), "break must not execute next statement")
}
...
func TestBasicTimingVariableCompatibility(t *testing.T) {
	c, step := bootClementinaToPrompt(t)
	for _, s := range []string{
		"10 TI=60", "20 TIME=120", "30 A=TI", "40 IF A<120 THEN POKE 512,1",
		"50 TI%=7", "60 DIM TI(2)", "70 TI(1)=9", "80 B=TI(1)+TI%",
		"90 IF B<>16 THEN POKE 512,2", `100 TI$="ABC"`, `110 IF TI$<>"ABC" THEN POKE 512,3`,
		"120 IF TI<120 THEN POKE 512,4", "130 TI=0", "140 POKE 513,123", "150 GOTO 150",
	} {
		typeLine(c, step, s)
	}
	typeLine(c, step, "RUN")
	editorTickN(c, step, 500_000)
	require_eq(t, 0, peek(c, 512), "TI scalar/typed variable/array coexistence")
	require_eq(t, 123, peek(c, 513), "TI variable test completed")
}
